...
public class TicketOffice {
    ...
    TicketOffice(
            SortedArrayList<Customer> customersSortedArrayList,
            SortedArrayList<Activity> activitySortedArrayList) {
     ...
     * @return return the customer if its successfully purchased or canceled the
     * ticket
     * There are several checks in this method in order to either buy And
     * Cancel the Ticket
     ...
     * 4. Customer can only register for the total number of activities
     * provided
     * 5. If the customer is canceling the ticket check if the customer is
     * already purchase the ticket or not
     * 6. Also update the customer and activity details accordingly whenever
     * customer is buying or cancel the ticket .
     * 7. If the customer is already registered for one of the activities
     * and buying the ticket again , will allow to purchase the ticket
     * again.
     * e.g. The no. of ticket left out of the no. of tickets available.
     * The total number of tickets purchased but the ticket for each
     * activity.
     */
    boolean buyAndCancelTicket(
        ...
        // If Customer is Purchasing the ticket isBuying will be true, or if cancelling
        // the ticket it will be false
        if (isBuying) {
            // check if ticket is available for the activity
            if (!checkIfTicketAvailableForActivity(
                    existingActivity,
            ...
            // then we need to allow the customer to buy the ticket, but we need to check if
            // ticket is available or not,then allow the customer to buy the ticket
            if (checkIfCustomerGotTicketForTheActivity(
                    existingCustomer,
                    existingActivity)) {
            ...
            // check if Customer has purchased any ticket in the past for the provided
            // activity
            if (!checkIfCustomerGotTicketForTheActivity(
                    existingCustomer,
                    existingActivity)) {
    ...
    int readNoOfTicketToBuyOrCancel(Scanner k) {
        ...
        int noOfTicket = 0;
        try {
            noOfTicket = Integer.parseInt(k.next());
     ...
     * @return boolean
     * Check if the ticket is available or not ,return false if not
     * available
     */
    public boolean checkIfTicketAvailableForActivity(
            Activity activity,
            int noOfTicketToBuy) {
        for (Activity act : activitySortedArrayList) {
            if (act.getActivityName().equalsIgnoreCase(activity.getActivityName())) {
     ...
     * @param activity object as input
     * @return boolean
     * Check if the customer has already purchased the ticket for the
     * provided activity
     */
    public boolean checkIfCustomerGotTicketForTheActivity(
            Customer customer,
            Activity activity) {
     ...
     * @param customer object as input
     * @param activity object as input
     *                 This method will print the customer a letter which is being
     *                 called when there is no ticket is available
     */
    public void printLetter(
            ...
            Activity activity) {
        letter.print("Dear ");
        letter.print(customer.getFirstName() + " " + customer.getLastName());
        letter.println(",");
        letter.println(
                "There is no more ticket available for the activity " +
                        activity.getActivityName() +
                        ", Please try for another activity. We will get back to you, once ticket is available.");
        letter.println("Thank you ");
    }

    /**
     * When user purchase the ticket for the first time or purchasing the another
     * ticket for the same activity
     * Update the customer details with activity registered and number of ticket
     * purchased
    ...
    public boolean buyTicket(
            Customer customer,
            Activity activity,
            int numOfTicketBuying) {
        boolean flag = false;
        if (customer.getNoOfTicketForActivity().get(activity) == null) {
            customer.getNoOfTicketForActivity().put(activity, numOfTicketBuying); // add details about this activity and
            // number of ticket
            customer.setNoOfActivityRegistered(
                    customer.getNoOfActivityRegistered() + 1); // Increment number of activity registered
            flag = true;
            ...
            customer
                    .getNoOfTicketForActivity()
                    .put(activity, oldValue + numOfTicketBuying); // increment number ticket purchased
            flag = true;
        }
        activity.setNoOfTicketAvailableForActivity(
                activity.getNoOfTicketAvailableForActivity() - numOfTicketBuying); // update the activity object
        return flag;
    }

    /**
     * When user cancel the ticket for the provided activity,
     * First check if total no. ticket bought is greater than 1 ,in this case just
     * decrement the value of numberOfTicketBoughtEachActivity
     * And, increment the value of number Of Ticket Available PerActivity
     * But, if number Of Ticket Bought for Activity was 1 , in this case update the
     * value of totalNumberOfActivityRegistered
    ...
    public boolean cancelTicket(
            Customer customer,
            Activity activity,
            int numOfTicketCancelling) {
        int oldValue = customer.getNoOfTicketForActivity().get(activity);

        if (oldValue < numOfTicketCancelling) {
            System.out.println(
                    "The number of ticket being cancelled is not matching the number of tickets being purchased by the customer!");
            return false;
        } else if (oldValue > numOfTicketCancelling) {
            customer
                    .getNoOfTicketForActivity()
                    .put(activity, oldValue - numOfTicketCancelling);
        } else { // In this case customer is cancelling all the tickets he has purchased for the
            // activity, he is being left with 1 ticket
            customer.getNoOfTicketForActivity().remove(activity); // remove the activity object from the map<Activity,
            // Integer>
            customer.setNoOfActivityRegistered(
                    customer.getNoOfActivityRegistered() - 1); // reduce the number of registered activity
        }
        // update the activity object , the number of tickets available
        activity.setNoOfTicketAvailableForActivity(
                activity.getNoOfTicketAvailableForActivity() + numOfTicketCancelling);
        return true;
    }
}
