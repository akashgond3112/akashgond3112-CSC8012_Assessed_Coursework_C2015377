...
public class TicketOffice {
  ...
  boolean buyAndCancelTicket(
    ...
    if (noOfTicketToBuyOrCancel <= 0) {
      System.out.println(
        "Your input seems to be incorrect, Please enter a valid ticket number you want buy."
      );
      clerk.println(
        "Your input seems to be incorrect, Please enter a valid ticket number you want buy."
      );
    ...
    if (existingActivity == null) {
      System.out.println(
        ...
        " doesn't exist, Please try entering the correct activity from the below list."
      );
      clerk.println(
        "The provided activity : " +
        activity.getActivityName() +
    ...
    if (isBuying) {
      ...
      if (
        !checkIfTicketAvailableForActivity(
          existingActivity,
      ...
      if (
        checkIfCustomerGotTicketForTheActivity(
          existingCustomer,
          existingActivity
        )
      ) {
      ...
      if (
        existingCustomer.getNoOfActivityRegistered() ==
        NUMBER_OF_ALLOWED_REGISTRATION
      ) {
        System.out.println(
          "Hi " +
          existingCustomer.getLastName() +
  ...
  Customer readCustomerNames(Scanner k) {
    System.out.println(
      "Enter person’s first name and last name," + " and press Enter"
    );
  ...
  public Activity checkActivityExist(Activity activity) {
    for (Activity act : activitySortedArrayList) {
      if (
        act.getActivityName().equalsIgnoreCase(activity.getActivityName())
      ) return act;
    }
    return null;
  }
  ...
  public Customer checkIfCustomerExist(Customer customer) {
    for (Customer cust : customersSortedArrayList) {
      if (
        cust.getFirstName().equalsIgnoreCase(customer.getFirstName()) &&
  ...
  public boolean buyTicket(
    ...
    if (customer.getNoOfTicketForActivity().get(activity) == null) {
      ...
      customer.setNoOfActivityRegistered(
        customer.getNoOfActivityRegistered() + 1
      ); // Increment number of activity registered
      flag = true;
    ...
    activity.setNoOfTicketAvailableForActivity(
      activity.getNoOfTicketAvailableForActivity() - numOfTicketBuying
  ...
  public boolean cancelTicket(
    ...
    if (oldValue < numOfTicketCancelling) {
      ...
      customer.setNoOfActivityRegistered(
        customer.getNoOfActivityRegistered() - 1
      ); // reduce the number of registered activity
    }
